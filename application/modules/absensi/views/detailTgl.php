...
<h4 class="font-700 mb-n1 title_infodet"><?php echo $tgl;?></h4>
 ...
 <table class="entry2" width="100%">
    ...
    <tr>
        <td>Lama lembur</td><td><?php echo $this->tanggal->sebutanWaktu(substr($db->lembur,0,5))?> , Terhitung <?php echo $this->tanggal->sebutanWaktu(substr($db->lembur_terhitung,0,5))?> </td>
    </tr>
    <?php 
    ...
    <tr>
        <td>Uang lembur</td><td>Rp <?php echo number_format($db->n_lembur,0,",",".")?> 
        <?php
    if($db->n_uang_makan){
        echo br()."<i class='text-black'>(".$db->ket_lembur.")</i>";
    }?>
    </td>
    </tr>
    <tr>
        <td>Uang makan</td><td>Rp <?php echo number_format($db->n_uang_makan,0,",",".")?>
