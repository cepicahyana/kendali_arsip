...
foreach($dataKomen as $kom){ 
    if($kom->id_sender!=$this->m_reff->idu()){
        $komentator = $this->m_reff->goField("admin","owner","where id_admin='".$kom->id_sender."' ");
        $jk_komen   = "cs".$this->m_reff->goField("admin","jk","where id_admin='".$kom->id_sender."' ");
    }else{
    ...
    $(window).scroll(function(){
      if ($(window).scrollTop() == $(document).height() - $(window).height()){
       ...
       var param = {limit:limit,<?php echo $this->m_reff->tokenName()?>:token};
        $.ajax({
          type:'post',
          url:'<?php echo base_url()?>tanya_admin/page_status',
          data: param,
          dataType:"json",
          async: true,
