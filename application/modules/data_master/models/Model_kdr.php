...
class Model_kdr extends CI_Model  {
    ...
    function insert(){
        ...
        $this->db->insert("data_kordokter");
        $var["token"]=$this->m_reff->getToken();
        $this->m_reff->log("tambah akun dokter","data");
        return $var;
    }
    ...
    function update(){
        ...
        $this->db->set($form);
        // $this->db->set("nip",$this->input->post("f[kode]"));
        $this->db->update("data_kordokter");
        $var["token"]=$this->m_reff->getToken();
        $this->m_reff->log("update akun dokter","data");
        return $var;
    }
    ...
    function cek(){
        ...
        return $this->db->get("data_kordokter")->num_rows();
    }
    ...
    function cek_pass(){
        ...
        $this->db->where("password",md5($this->m_reff->san($this->input->post("password"))));
        $this->db->where("id!=",$this->m_reff->san($this->input->post("id")));
        $res = $this->db->get("data_kordokter")->num_rows();
        if($res){
            return 1;
        }
