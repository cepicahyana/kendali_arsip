		 	...
		 	<div class="col-md-4 mb-3">
 				<?php
					...
					foreach($db as $db){
				  	...
				  	echo form_dropdown("istana", $dtIstana, "", "class='text-black form-control' onchange='getData()'") ?>
 			</div>
			<?php endif ;
 			...
 			<div class="col-md-4 mb-3">
 				<?php
					...
					foreach($db as $db){
				  	...
				  	echo form_dropdown("bagian", $dtBidang, null, "class='text-black form-control' onchange='getData()'") ?>
 			</div>
 ...
 <script>
	 function getData() {
		loading("dataAbsen");
		var url   = "<?php echo site_url("lembur/range_absen");?>";
		var periode = $("#periode").val();
		var istana = $("[name='istana']").val();
		var bagian = $("[name='bagian']").val();
		var jk = $("[name='jk']").val();
		var param = {jk:jk,istana:istana,bagian:bagian,periode:periode,<?php echo $this->m_reff->tokenName()?>:token};
				$.ajax({
 	...
 	function detailTgl(nip,tgl) {
		$("#mdl_formSubmit").modal();
		$("#mdlValue").html("mohon tunggu...");
		var url   = "<?php echo site_url("lembur/detailTgl");?>";
