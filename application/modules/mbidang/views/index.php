 ...
 <script type="text/javascript">
 	...
 	var dataTable = $('#table').DataTable({
 		...
 		"ajax": {
 			"url": "<?php echo site_url('mbidang/getData'); ?>",
 			"type": "POST",
 			"data": function(data) {
 				data.jk = $('[name="jk"]').val();
...
function tambah(){
    ...
    $.post("<?php echo base_url() ?>mbidang/form_tambah", function(data, status){
    $("#mdlValue").html(data);
   });
}
</script>
...
function simpan() {
	$("#mdl_formSubmit").modal();
	$("#mdlValue").html("mohon tunggu...");
	var url   = "<?php echo site_url("mbidang/detail");?>";
...
function detail(nip) {
 ...
 <script>
 	function edit(id) {
 		$("#mdl_formSubmit").modal();
 		$("#mdlValue").html("mohon tunggu...");
 		$.post("<?php echo base_url() ?>mbidang/form_edit", {
 			id: id
 ...
 <script>
 	function hapus(id, nama) {
 		alertify.confirm("<center> " + nama + "<br>Hapus ? </center>", function() {
 			$.post("<?php echo base_url() ?>mbidang/hapus", {
 				id: id
 			}, function(data, status) {
