...
<div class="breadcrumb-header justify-content-between">
	<div>
		<h4 class="content-title mb-2">GRAFIK MELAKUKAN TES</h4>
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb">
				<li class="breadcrumb-item ">
					<div class="text-white"> Pilih Grafik :


						<input type="text" id="range" size="30" >
						<select style="height:26px" id="type" onchange="goGrafik()">
							<option value="1">Harian</option>
							<!-- <option value="2">Mingguan</option>  -->
							<option value="3">Bulanan</option>
						</select>
						<button class='btn btn-sm bg-warning' onclick="goGrafik()">submit</button>
					</div>
				</li>
			</ol>
		</nav>
		...
		<div class="card overflow-hidden">
			<div class="card-header bg-transparent pd-b-0 pd-t-20 bd-b-0">
				<div class="d-flex justify-content-between">
					<h4 class="card-title mg-b-10 title-grafik">Grafik</h4>
			...
			<div class="card-body pd-y-7">
				...
				<div id="table-rekap-periksa"></div>
			</div>
		</div>
	</div>
...
<script>
	...
	function goGrafik() {
		...
		var range = $("#range").val();
		if (type == 1) {
			var title = "Grafik Harian";
		} else if (type == 2) {
			var title = "Grafik Mingguan";
		} else {
			var title = "Grafik Bulanan";
		}

		$(".title-grafik").html(title);
		var url = "<?php echo site_url("pic/test/goGrafik"); ?>";
		var param = {
			type: type,
			range: range,
	...
	function goTable() {
		...
		var type = $("#type").val();
		var range = $("#range").val();
		var url = "<?php echo site_url("pic/test/goTable"); ?>";
		var param = {
			type: type,
			range: range,
			<?php echo $this->m_reff->tokenName() ?>: token
		...
		$.ajax({
			...
			success: function(val) {
				token = val['token'];
				$("#table-rekap-periksa").html(val['data']);
			}
		});
	}
	...
	// 	var range = $("#range").val();
	// 	var url = "<?php echo site_url("pic/test/count_periode"); ?>";
	// 	var param = {
	// 		range: range,
	// 		<?php echo $this->m_reff->tokenName() ?>: token
	// 	};
