...
class Model extends CI_Model
{
	...
	function set_evaluator(){
		$nip			=	$this->input->post("evaluator");
		$org 			=	$this->input->post("org");
		$data = $this->m_reff->clearkomaray($org);

		$this->db->where_in("id",$data);
		$this->db->set("nip_evaluator",$nip);
		return $this->db->update("data_pegawai");
	}
	function data_pns(){
		$nama = $this->input->post("search");
		$this->db->select("nip as id,CONCAT(nama,' - ',jabatan) as text");
		$this->db->where("jenis_pegawai",1);
		$this->db->like("nama",$nama);
		return $this->db->get("data_pegawai")->result();
	}
	function evaluator(){
		$this->db->select("distinct(nip_evaluator)");
		$this->db->where("nip_evaluator IS NOT NULL");
		// $this->db->group_by("nip_evaluator");
		return $this->db->get("data_pegawai")->result();
	}
	function nama_evaluator($nip){
	...
	private function _getData()
	{
		...
		$evaluator	= $this->m_reff->san($this->input->post("evaluator"));
		if($evaluator=="kosong") {
			$this->db->where("nip_evaluator IS NULL");
		}elseif($evaluator) {
			$this->db->where("nip_evaluator",$evaluator);
		}
