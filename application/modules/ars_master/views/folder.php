        		...
        		<table id='table' width="100%" class="tabel black table-striped table-bordered table-hover dataTable">
				  	<thead>
				  		<tr>
							...
							<th class='thead' >Box </th> 
							<th class='thead' >Tahun </th> 
	...
	function hapus(id,akun){
		swal({
			...
			if (willDelete) {
				...
				var url   = "<?php echo site_url("ars_master/hapus_folder");?>";
      ...
      var  dataTable = $('#table').DataTable({ 
        ...
        buttons: [
           ...
           {  
				...
				className: "btn  btn-secondary-light",
				title: 'Master Folder',
				messageTop: 'Export Master Folder | <?php echo date("d/m/Y H:i"); ?>',
				filename: 'Export-Master-Folder',
				customize: function(xlsx) {
					var source = xlsx.xl['workbook.xml'].getElementsByTagName('sheet')[0];
					source.setAttribute('name', 'Master Folder');
				}
 			},
        {
        ...
        "ajax": {
        	"url": "<?php echo site_url('ars_master/getData_folder');?>",
        	"type": "POST",
        	"data": function ( data ) {
      ...
      {	 
        ...
        var url   = "<?php echo site_url("ars_master/form_folder");?>";
        var param = {<?php echo $this->m_reff->tokenName()?>:token,id:id};
        $.ajax({
         type: "POST",dataType: "json",data: param, url: url,
         success: function(val){
