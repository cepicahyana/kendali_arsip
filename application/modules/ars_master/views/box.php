	...
	function hapus(id,akun){
		swal({
			...
			if (willDelete) {
				...
				var url   = "<?php echo site_url("ars_master/hapus_box");?>";
      ...
      var  dataTable = $('#table').DataTable({ 
        ...
        buttons: [
           ...
           {  
				...
				className: "btn  btn-secondary-light",
				title: 'Master Box',
				messageTop: 'Export Master Box | <?php echo date("d/m/Y H:i"); ?>',
				filename: 'Export-Master-Box',
				customize: function(xlsx) {
					var source = xlsx.xl['workbook.xml'].getElementsByTagName('sheet')[0];
					source.setAttribute('name', 'Master Box');
				}
 			},
        {
        ...
        "ajax": {
        	"url": "<?php echo site_url('ars_master/getData_box');?>",
        	"type": "POST",
        	"data": function ( data ) {
      ...
      {	 
        ...
        var url   = "<?php echo site_url("ars_master/form_box");?>";
        var param = {<?php echo $this->m_reff->tokenName()?>:token,id:id};
        $.ajax({
         type: "POST",dataType: "json",data: param, url: url,
         success: function(val){
