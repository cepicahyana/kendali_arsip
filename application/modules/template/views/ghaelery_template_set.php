<div class="col-md-12 alert alert-info"> <center><b>  <?php echo strtoupper($this->m_reff->goField("tr_jenis_kegiatan","nama","where id='".$this->input->post("id_acara")."' "))?></b></center> </div>
					...
					<img class="img-fluid img-thumbnail" src="<?php echo $poto_template;?>" width="300px" width="300px"> </a><br> 
					 <div class="btn-group">
					 <button class="btn btn-danger btn-sm feather icon-trash-2" style="min-width:20px" onclick="hapus_template(`<?php echo $this->m_reff->encrypt($val->id)?>`,`<?php echo $val->nama?>`,`<?php echo $id_acara?>`)"> Hapus </button>
					 <button class="btn btn-info btn-sm feather icon-edit " style="min-width:100px" onclick="edit_template(`<?php echo $val->id?>`)"> Edit Template </button>
					  </div>
					 </center><br>
					 </div>
					...
					<center>
					<a class="btn btn-primary fas fa-plus-circle" href="<?php echo base_url()?>template/create?id=<?php echo $id_acara;?>" target="_blank"> Buat template baru</a>
					</center>
					</div>
