		...
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb">
				<li class="breadcrumb-item"><a href="#">
						Data yang muncul dibawah merupakan pertanyaan yang ditanggapi oleh anda atau belum 
					ditanggapi oleh dokter lain.</a></li>
...
maxlength="250" name="update_status" class="font-22" placeholder="ketik disini..."></textarea>
 ...
 <a href="javascript:kirim_status()" class="btn btn-m btn-full mb-3 rounded-xl text-uppercase font-900 shadow-s bg-highlight">Kirim</a>
	...
	<script>
		...
		// 	var url = "<?php echo site_url("dokter/getNewReplay"); ?>";
		// 	var param = {
		...
		function reload_status() {
			...
			var load = loading("shareInfo");
			$("#shareInfo").html(load);
			var url = "<?php echo site_url("dokter/reload_status"); ?>";
			var param = {
				<?php echo $this->m_reff->tokenName() ?>: token
			};
			$.ajax({
		...
		function kirim_status() {
			...
			var url = "<?php echo site_url("dokter/kirim_status"); ?>";
			var param = {
				msg: msg,
				<?php echo $this->m_reff->tokenName() ?>: token
			};
		...
		function kirim_balasan() {
			...
			var url = "<?php echo site_url("dokter/kirim_balasan"); ?>";
			var param = {
				msg: msg,
				idm: idm,
				<?php echo $this->m_reff->tokenName() ?>: token
		...
		function lihat_obrolan(id) {
			var url = "<?php echo site_url("dokter/lihat_obrolan"); ?>";
			var param = {
				id: id,
				<?php echo $this->m_reff->tokenName() ?>: token
			};
	...
	<script>
		function replay(id, msg) {
			...
			// $('#replayID').val(id);
			//replayID = id;
			var url = "<?php echo site_url("dokter/tanggapi"); ?>";
					var param = {
						id: id,
						msg:msg,
						<?php echo $this->m_reff->tokenName() ?>: token
	...
	<script>
		...
		function hapus_sts(id) {
			...
			swal({
				...
				if (willDelete) {
					...
					var url = "<?php echo site_url("dokter/hapus_status"); ?>";
					var param = {
						id: id,
						<?php echo $this->m_reff->tokenName() ?>: token
					};
		...
		function akhiri(id) {
			...
			swal({
				...
				if (willDelete) {
					var url = "<?php echo site_url("dokter/akhiri_obrolan"); ?>";
					var param = {
						id: id,
						<?php echo $this->m_reff->tokenName() ?>: token
					};
