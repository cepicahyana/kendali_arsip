...
$this->db->order_by("oleh", "asc");
$this->db->order_by("tgl_respon", "asc");
$this->db->limit(10, 0);
$this->db->where("sts", "0");
$this->db->where("(id_dokter = " . $idDokter . " OR (id_dokter is null or id_dokter = ''))");
                        ...
                        <div class="media d-block d-sm-flex">
                            ...
                            <div class="media-body">
                                ...
                                <div class="align-self ml-auto">
                                   <?php  $res = $val->id_dokter;
                                   if(!$res){
                                       echo "<i class='badge badge-info'  onclick='replay(`".$val->id."`,`".$val->msg."`)'>belum ditanggapi</i>";
                                   }else{
                                    echo "<button class='btn btn-sm btn-warning' onclick='akhiri(`".$val->id."`)'>akhiri percakapan</button>";
                                   }
                                   ?>
...
echo '<a class="text-blue2-dark " style="float:right;margin-right:5px" href="javascript:replay(`'.$val->id.'`,`'.$val->msg.'`)" class="color-theme">
<i class="fa fa-share pr-2"></i>'.$jml.' percakapan  </a>';
                                        }else{
