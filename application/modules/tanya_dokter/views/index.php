 ...
 function reload_chat(){
	 var id = "<?php echo $this->input->post('id');?>"; 
	  ...
	  $.post("<?php echo base_url()?>tanya_dokter/chat_list",{id:id}, function(data, status){
		  $("#chatlist").html(data);
	  }); 
 }
   ...
   function type_teks(e)
	...
	var inField=$("#type_teks").val();
	 ...
	 var receiver = "<?php echo $this->input->post('id');?>";	 
	  ...
	  open=true;
	 $.ajax({
		 url:'<?php echo site_url("tanya_dokter/saveChat"); ?>',
		 data:{receiver:receiver,msg:text},
		 method:"POST",
		 dataType:"JSON",
		 beforeSend: function() {
 ...
 function scroll(){
  ...
  function replays(){ 
	 ...
	 $.ajax({
		 url:'<?php echo site_url("tanya_dokter/chat_replay"); ?>',
		 data:{sender:sender},
		 method:"POST",
		 dataType:"JSON",
		 beforeSend: function() {
