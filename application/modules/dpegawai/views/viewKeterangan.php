...
$nik     = isset($db->nik)?($db->nik):"";


$peg     = $this->m_reff->data_pegawai($nik);
if(!isset($peg)){   return false; }
$nik     = isset($peg->nik)?($peg->nik):"";

if($hasil=="+"){
    $hasil_text = "<h5 class='text-danger'>Positif (+)</h5>
    <a class='text-primary' href='".site_url("download")."?f=".$this->m_reff->encrypt($db->file)."'> 
...
<div class="modal-content">  
					...
					<center><b>hasil tes anda dinyatakan : </b><br>
...
<!-- <div class="col-lg-12 p-1"> <span>Lainnya: <input type="text" name="perawatan[]" class='form-control'> </span> </div> -->
 ...
 <b>Pilih kondisi yang anda rasakan:</b> 
 <?php
 $data = $this->db->get("tr_indikasi")->result();
foreach($data as $val){
	
echo '<div class="col-lg-12 p-1 alert alert-info"> <label class="ckbox"><input  type="checkbox" name="indikasi[]" value="'.$val->id.'"><span  style="line-height:20px">'.$val->indikasi.'</span></label> </div>';
}
?>
...
<div class="col-lg-12 p-1"> <span>Deskripsikan kondisimu saat ini : <textarea type="text" name="ket" class='form-control'></textarea> </span> </div>
<br>


 <div class="col-lg-12 p-1"><center>
<button class="btn btn-success" onclick="submitForm('modal_ket')"><i class="fa fa-save"></i> simpan</button>
</center>
</div>
